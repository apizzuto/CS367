...
// Files:            WordCloudGenerator.java, ArrayHeap.java, BSTnode.java,
//						BSTDictionary.java, DictionaryADT.java, 
//						DuplicateException.java, BSTDictionaryEditor.java,
//						KeyWord.java, Prioritizable.java, PriorityQueueADT.java
...
public class WordCloudGenerator {
    ...
    public static void main(String[] args) {
        ...
        //Check to see if input file exists and is readable
        try { 
        ...
        //Check to see if ignore file exists and is readable
        try { 
        ...
        try {
        		...
        		//If we cannot write to this file, warn user and quit
        ...
        while (in.hasNext()) {
            ...
            for (String word : words) {
                ...
                KeyWord kWord = new KeyWord(word);
                kWord.increment();
                if ( ignore.lookup(word)==null ) {
        			//If not in Ignore Dictionary, try to add
        				try {
        					dictionary.insert(kWord);
        				} catch (DuplicateException d) {
        					//If already in the dictionary, lookup
        					//and increment its occurrences
        					dictionary.lookup(kWord).increment();
        				}
        			}
            }
        ...
        System.out.println("# keys: " + dictSize);
        //Prints the number of keys in the dictionary
        
        float avgPath = (float)dictionary.totalPathLength()/dictSize;
        System.out.println("avg path length: " + avgPath);
        //Prints out the average path length
        
        float linPath = (float)(dictSize+1)/2;
        System.out.println("linear avg path: " + linPath);
        //Prints out the linear average path
        
        
        //Next, we put the dictionary into a priority queue
        PriorityQueueADT<KeyWord> pQueue = new ArrayHeap<KeyWord>();
        //Creates empty priority queue implemented as an array heap
        Iterator<KeyWord> itrt = dictionary.iterator();
        //Creates BSTDictionaryIterator
        while(itrt.hasNext() ) {
        		//Iterate over the dictionary
        		pQueue.insert(itrt.next());
        }
        
        //Next, we create a list of KeyWords of appropriate length
        if(pQueue.size() < maxWords ) {
        		maxWords = pQueue.size();
        		//maxWords cannot be more than the number of words we have
        }
        DictionaryADT<KeyWord> mostPop = new BSTDictionary<KeyWord>();
        //Creates dictionary for the most popular words that we are interested in
       
        for (int i = 0; i < maxWords; i++ ) {
        		try{
        			mostPop.insert(pQueue.removeMax());
        		} catch (DuplicateException d) {
        			// if there is a duplicate, we'll just ignore it
        		}
        }
        
        //Finally, we generate the html output file
        generateHtml(mostPop, out);
